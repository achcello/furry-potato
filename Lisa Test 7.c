...
void pre_auton()
{
	string mainBattery, backupBattery, expBattery = "";
	...
	while (bIfiRobotDisabled != 1) { //Ensures this code will run ONLY when the robot is disabled
		...
		/*else if (lcdScreen == 6) {
			displayLCDString(0, 0, "Exp: ");
			sprintf(expBattery, "%1.2f%c", BackupBatteryLevel/1000.0, 'V');    //Build the value to be displayed;
